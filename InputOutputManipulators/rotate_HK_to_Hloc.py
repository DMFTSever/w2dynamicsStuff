...
sys.path.insert(0,sys.argv[0].replace('/InputOutputManipulators/rotate_HK_to_Hloc.py','/General'))
...
for i in range(0,Natoms):
    if np.allclose(hkmean_pa[i]-np.diag(np.diagonal(hkmean_pa[i])), np.zeros_like(hkmean_pa[i])):
        print("Atom "+str(i+1) + " already diagonal. No diagonalisation necessary for this atom")
